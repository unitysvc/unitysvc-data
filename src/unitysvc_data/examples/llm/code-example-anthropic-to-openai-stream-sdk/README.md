...
is_active = true
is_public = true
meta = { variant = "Anthropic-style (streaming)", requirements = ["openai", "anthropic"], min_expected_metrics = { input_tokens = 1, output_tokens = 1 } }
...
### v2 — read raw Anthropic stream events

- Iterates `client.messages.create(..., stream=True)` events directly instead
  of `client.messages.stream()`: the helper's accumulator crashes when a
  translated stream deviates from Anthropic's exact event shapes.

### v3 — tolerate a usage-only final chunk
