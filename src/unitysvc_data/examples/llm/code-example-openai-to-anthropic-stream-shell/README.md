+++
preset_name = "llm_code_example_openai_to_anthropic_stream_shell"
...
is_active = true
is_public = true
meta = { variant = "OpenAI-style (streaming)", output_contains = "example ok", min_expected_metrics = { input_tokens = 1, output_tokens = 1 } }
...
- **otherwise** — call the **gateway** in openai-shape (OpenAI chat-completions API (`/v1/chat/completions`)); the
  gateway streams server-sent events and translates to the Anthropic upstream and back.
...
- Streaming variant — reads `curl -N` frame by frame.
